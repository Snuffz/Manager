...
class AntieveryoneCmd extends Command {
  constructor (client) {
    super(client, {
      name: "antieveryone",
      description: "set the anti-everyone strikes",
      category: "Automod",
      usage: "<strikes>",
      guildOnly: true,
      aliases: ["antiateveryone", "anti-everyone", "anti-ateveryone"],
...
if(numstrikes<0 || numstrikes>PunishmentManager.MAX_STRIKES)
{
  ...
  return;
        ...
        settings.antiEveryone = numstrikes;
        await settings.save().catch(e => this.client.logger.log(e, "error"));
        const also = PunishmentManager.useDefaultSettings(message.guild);
        return reply(`${this.client.config.emojis.success} Users will now receive \`${numstrikes}\`strikes for sending @\u0435veryone/here.${also ? PunishmentManager.DEFAULT_SETUP_MESSAGE : ""}`);
    })
    
  }
}

module.exports = AntieveryoneCmd;