...
class ServerLog extends Command {
  constructor (client) {
    super(client, {
      name: "serverlog",
      description: "define the server log channel",
      category: "Settings",
      usage: "[#channel or OFF]",
      guildOnly: true,
  ...
  async run (message, args, reply) {
    Settings.findOne({
      ...
      function showCurrentChannel(){
        const tc = message.guild.channels.cache.get(settings.serverLog);
        if(tc==undefined)
        {
          reply(`${message.client.config.emojis.success} The server logs is not set on this server.`);
          return;
        } 
        else reply(`${message.client.config.emojis.success} Server logs are currently appearing in ${tc.toString()}
${tc.permissionsFor(message.guild.me).missing(required_perms).length==0?"":message.client.config.emojis.warning+required_error(tc.toString())}`)
    ...
    if(tc==undefined)
    {
      settings.serverLog = "off";
      await settings.save().catch(e => message.client.logger.log(e));
      reply(`${message.client.config.emojis.success} The server logs will no longer appear.`);
      return;
    }
    else 
    settings.serverLog = tc.id;
    await settings.save().catch(e => message.client.logger.log(e));
    reply(`${message.client.config.emojis.success} Server Logs will now be sent in ${tc.toString()}`);
    return;
      }
    });
  }
