...
class MessagelogCmd extends Command {
  constructor (client) {
    super(client, {
      name: "messagelog",
      description: "define the message log channel",
      category: "Settings",
      usage: "[channel]",
      guildOnly: true,
  ...
  async run (message, args, reply) {
    Settings.findOne({
      ...
      function showCurrentChannel(){
        const tc = message.guild.channels.cache.get(settings.messageLog);
        if(tc==undefined)
        {
          reply(`${message.client.config.emojis.success} The message logs is not set on this server.`);
          return;
        } 
        else reply(`${message.client.config.emojis.success} Message logs are currently appearing in ${tc.toString()}
${tc.permissionsFor(message.guild.me).missing(required_perms).length==0?"":message.client.config.emojis.warning+required_error(tc.toString())}`)
    ...
    if(tc==undefined)
    {
      settings.messageLog = "off";
      await settings.save().catch(e => message.client.logger.log(e));
      reply(`${message.client.config.emojis.success} The message logs will no longer appear.`);
      return;
    }
    else 
    settings.messageLog = tc.id;
    await settings.save().catch(e => message.client.logger.log(e));
    reply(`${message.client.config.emojis.success} Message Logs will now be sent in ${tc.toString()}`);
    return;
      }
    });
  }
